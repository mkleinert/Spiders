...
infile = '/Users/mkleinert/Spiders/crawl_BA/topbeer_desc.csv'
stop_words = text.ENGLISH_STOP_WORDS.union(['beer','bottle','glass','head','like','ve','flavor','taste','tree','younger','elder','stout','ipa','julius','house','just','really','heady','drink','poured','abraxas'])
ngram_vectorizer = CountVectorizer(analyzer='word', ngram_range=(1, 1), min_df=1,stop_words=stop_words)

style_df = pd.read_csv(infile)
grouped = style_df.groupby('beerName').agg(sum)

mypalette24 = viridis(100)
appended_data = []
...
for idx,style in enumerate(grouped.index):
    ...
    word_freq_df['beerName'] = grouped.index[idx]
...
p = figure(title='Top 10 Beers Descriptors',plot_width=1000, plot_height=1000,x_range=(0, 300),  y_range=(.0055, .04))
color = mypalette24
p.scatter(x='occurrences', y='frequency', size=8, source=source, color=color,legend='beerName') #mypalette24[idx]
